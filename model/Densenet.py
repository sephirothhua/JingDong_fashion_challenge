        ...
        if include_top:
            if blocks == [6, 12, 24, 16]:
                weights_path = get_file(
                    os.path.join(model_path,'densenet121_weights_tf_dim_ordering_tf_kernels.h5'),
                    DENSENET121_WEIGHT_PATH,
                    cache_subdir='./model_save',
                    file_hash='0962ca643bae20f9b6771cb844dca3b0')
            elif blocks == [6, 12, 32, 32]:
                weights_path = get_file(
                    os.path.join(model_path,'densenet169_weights_tf_dim_ordering_tf_kernels.h5'),
                    DENSENET169_WEIGHT_PATH,
                    cache_subdir='./model_save',
                    file_hash='bcf9965cf5064a5f9eb6d7dc69386f43')
            elif blocks == [6, 12, 48, 32]:
                weights_path = get_file(
                    os.path.join(model_path,'densenet201_weights_tf_dim_ordering_tf_kernels.h5'),
            ...
            if blocks == [6, 12, 24, 16]:
                weights_path = get_file(
                    os.path.join(model_path,'densenet121_weights_tf_dim_ordering_tf_kernels_notop.h5'),
                    DENSENET121_WEIGHT_PATH_NO_TOP,
                    cache_subdir='./model_save',
                    file_hash='4912a53fbd2a69346e7f2c0b5ec8c6d3')
            elif blocks == [6, 12, 32, 32]:
                weights_path = get_file(
                    os.path.join(model_path,'densenet169_weights_tf_dim_ordering_tf_kernels_notop.h5'),
                    DENSENET169_WEIGHT_PATH_NO_TOP,
                    cache_subdir='./model_save',
                    file_hash='50662582284e4cf834ce40ab4dfa58c6')
            elif blocks == [6, 12, 48, 32]:
                weights_path = get_file(
                    os.path.join(model_path, 'densenet201_weights_tf_dim_ordering_tf_kernels_notop.h5'),
...
def DenseNet121(include_top=True,
                ...
                pooling=None,
                classes=1000):
    if(weights is 'imagenet' and include_top):
        print('Loading weigts from {}'.format(DENSENET121_WEIGHT_PATH))
    elif(weights is 'imagenet' and ~include_top):
        print('Loading weigts from {}'.format(DENSENET121_WEIGHT_PATH_NO_TOP))
    else:
        print('Loading weigts from {}'.format(weights))
    return DenseNet([6, 12, 24, 16],
                    include_top, weights,model_path,
...
def DenseNet169(include_top=True,
                ...
                pooling=None,
                classes=1000):
    if(weights is not 'imagenet' and include_top):
        print('Loading weigts from {}'.format(DENSENET169_WEIGHT_PATH))
    elif(weights is not 'imagenet' and ~include_top):
        print('Loading weigts from {}'.format(DENSENET169_WEIGHT_PATH_NO_TOP))
    else:
        print('Loading weigts from {}'.format(weights))
...
def DenseNet201(include_top=True,
                ...
                classes=1000):
    if(weights is not 'imagenet' and include_top):
        print('Loading weigts from {}'.format(DENSENET201_WEIGHT_PATH))
    elif(weights is not 'imagenet' and ~include_top):
        print('Loading weigts from {}'.format(DENSENET201_WEIGHT_PATH_NO_TOP))
    else:
        print('Loading weigts from {}'.format(weights))
